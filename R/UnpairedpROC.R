...
fncUnpairedpROC <- function(){
  ...
  defaults <- list(# data tab
                   ...
                   initial.label2 = NULL, # independent
                   ...
                   initial.tab=0) # tab
  dialog.values <- getDialog("UnpairedpROC", defaults)
  
  
  initializeDialog(title=gettext("Unpaired ROC curves comparison", domain="R-RcmdrPlugin.ROC"), use.tabs=TRUE, tabs=c("dataTab", "smoothingTab", "aucTab", "ciTab", "optionsTab")) # tab
  ...
  labelBox <- variableListBox(generaldataFrame, Factors(), title=gettext("Outcome variable 1 (pick one)", domain="R-RcmdrPlugin.ROC"),
                              initialSelection=varPosn(dialog.values$initial.label, "factor"))
  label2Box <- variableListBox(generaldataFrame, Factors(), title=gettext("Outcome variable 2(pick one)", domain="R-RcmdrPlugin.ROC"),
                              initialSelection=varPosn(dialog.values$initial.label2, "factor"))
  ...
  onOK <- function(){
    ...
    putDialog ("UnpairedpROC", list(# data tab
                            ...
                            initial.label2 = label2, # independent
...
if (partialauc == "TRUE") {
  if (0 == length(partialto)) {
    errorCondition(recall=fncUnpairedpROC, message=gettext("You must set a partial AUC 'to' limit.", domain="R-RcmdrPlugin.ROC"))
    return()
  }
  partialto = as.numeric(partialto)
  partialfrom = as.numeric(partialfrom)
  if ((partialto < 0) | (partialto > percentupper)) {
    errorCondition(recall=fncUnpairedpROC, message=gettext("Partial AUC 'to' limit outside of range.", domain="R-RcmdrPlugin.ROC"))
    return()
  }
  if (0 == length(partialfrom)) {
    errorCondition(recall=fncUnpairedpROC, message=gettext("You must set a partial AUC 'from' limit.", domain="R-RcmdrPlugin.ROC"))
    return()
  }
  if ((partialfrom < 0) | (partialfrom > percentupper)) {
    errorCondition(recall=fncUnpairedpROC, message=gettext("Partial AUC 'from' limit outside of range.", domain="R-RcmdrPlugin.ROC"))
    return()
  }
...
if ((printthresrb == "custom") & (0 == length(customthres))) {
  errorCondition(recall=fncUnpairedpROC, message=gettext("Custom threshold for variable 1 should not be empty.", domain="R-RcmdrPlugin.ROC"))
  return()
}
if ((printthresrb == "custom") & (0 == length(customthres2))) {
  errorCondition(recall=fncUnpairedpROC, message=gettext("Custom threshold for variable 2 should not be empty.", domain="R-RcmdrPlugin.ROC"))
  return()
}
